...
项目是一个 agent-native CLI / Skill 编排层：研究问题 -> 识别策略 -> 数据审计 -> Stata/R/Python 执行 -> 静态质量核验 -> 论文写作质检和 session 管理。

## 2. 安装

如果 C 盘空间紧张，可以把 pip 缓存和临时目录放到空间更充足的磁盘。下面的路径只是示例，请换成自己的目录：

```powershell
New-Item -ItemType Directory -Force "D:\dev-cache\pip","D:\dev-cache\tmp" | Out-Null
...
- `outputs\min-wage-demo\verify_report.md`：方法证据和写作风险核验报告
...
| verify | `python scripts\cli.py verify --strategy DiD --analysis outputs\<name>` | 方法证据和写作风险闸门 |
...
样例流程不需要 API key。真实论文项目可以按需打开下面的增强能力：
...
Stata-first 是本项目的默认使用习惯，但脚手架生成不要求本机安装 Stata。
...
| OpenCode | `.opencode\skills\econ-paper-studio\SKILL.md` |
| Codex | `AGENTS.md` + `skill_loadout.yaml` |
| Cursor | `AGENTS.md` + `.cursor\rules\econ-paper-studio.mdc` |
| Coze | `integrations\coze\econ-paper-studio-tool-schema.json` + 受控 runner API |

如果要让 Claude Code 或 OpenCode 在任意论文项目里都能调用它，可以把仓库目录链接到对应 skills 目录。
...
cmd /c rmdir "%USERPROFILE%\.config\opencode\skills\econ-paper-studio"
```

Cursor 通常直接读取项目里的 `AGENTS.md` 和 `.cursor\rules\*.mdc`。Coze 不应直接执行本机 shell；应通过一个只暴露白名单命令的 runner API 调用 `econ-studio`。细节看 [docs/AGENT_INTEGRATIONS.md](./docs/AGENT_INTEGRATIONS.md)。
...
- [docs/AGENT_INTEGRATIONS.md](./docs/AGENT_INTEGRATIONS.md) - 多平台 agent 接入方式
